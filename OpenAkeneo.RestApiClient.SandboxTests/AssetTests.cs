...
public class AssetTests : IClassFixture<TestBase>
{
    ...
    {
        var result = await _fixture.Context.GetAssetFamilyListAsync();
    ...
    {
        var result = await _fixture.Context.GetAssetFamilyAsync(AssetFamilyCode);
    ...
    {
        var result = await _fixture.Context.GetAssetAttributeListAsync(AssetFamilyCode);
    ...
    {
        var result = await _fixture.Context.GetAssetAttributeAsync(AssetFamilyCode, AssetAttributeCode);
    ...
    {
        var result = await _fixture.Context.GetAssetAttributeOptionListAsync(AssetAttributeOptionFamilyCode, AssetAttributeOptionAttributeCode);
    ...
    {
        var result = await _fixture.Context.GetAssetAttributeOptionAsync(AssetAttributeOptionFamilyCode, AssetAttributeOptionAttributeCode, AssetAttributeOptionCode);
    ...
    {
        var result = await _fixture.Context.GetAssetListAsync(AssetFamilyCode);
    ...
    {
        var result = await _fixture.Context.GetAssetListFullAsync(AssetFamilyCode);
    ...
    {
        var count = 0;
        await foreach (var item in _fixture.Context.StreamAssetsAsync(AssetFamilyCode))
        {
            Assert.NotNull(item);
            count++;
        }
    ...
    public async Task GetAssetAsync_ReturnsAsset()
    {
        ...
        var listResult = await _fixture.Context.GetAssetListAsync(AssetFamilyCode, limit: 1);

        if (listResult.Assets.Count > 0)
        {
            var assetCode = listResult.Assets[0].Code;
            var result = await _fixture.Context.GetAssetAsync(AssetFamilyCode, assetCode);
        ...
        {
            // If no assets exist in the family, the test should inherently pass or throw an informative skip-like assert
    ...
    public async Task CreateOrUpdateAssetAsync_Lifecycle_CreateThenUpdateThenVerify()
    {
        ...
        var createResult = await _fixture.Context.CreateOrUpdateAssetAsync(AssetFamilyCode, asset);
        ...
        var updateResult = await _fixture.Context.CreateOrUpdateAssetAsync(AssetFamilyCode, updated);
